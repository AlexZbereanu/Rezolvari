...
@Service
class WeatherForecastService(private val timeService: TimeService, private val locationService: LocationSearchService) : WeatherForecastInterface {
    override fun getForecastData(location: String): WeatherForecastData {
        // determinare locationId
        val locationId: Int = locationService.getLocationId(location)

        // dacă locaţia nu a fost găsită, răspunsul va fi corespunzător
        if (locationId == -1) {
            return WeatherForecastData("","","","","",0,0,0,0,0)
        }
